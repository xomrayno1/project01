package com.tamnc.messaging.mapper;
...
import com.tamnc.kafka.model.CustomerAvroModel;
import com.tamnc.kafka.model.PaymentOrderStatus;
import com.tamnc.kafka.model.PaymentRequestAvroModel;
import com.tamnc.kafka.model.PaymentResponseAvroModel;
import com.tamnc.kafka.model.RestaurantApprovalRequestAvroModel;
import com.tamnc.kafka.model.RestaurantApprovalResponseAvroModel;
import com.tamnc.kafka.model.RestaurantOrderStatus;
...
public class OrderMessagingDataMapper {
	public PaymentResponse paymentResponseAvroModelToPaymentResponse(
		...
		return PaymentResponse.builder().id(paymentResponseAvroModel.getId())
				...
				.failureMessages(paymentResponseAvroModel.getFailureMessages()).build();
	}
	...
	public RestaurantApprovalRequestAvroModel orderApprovalEventToRestaurantApprovalRequestAvroModel(String sagaId,
		...
		return RestaurantApprovalRequestAvroModel.newBuilder().setId(UUID.randomUUID().toString()).setSagaId(sagaId)
				...
				.setProducts(orderApprovalEventPayload.getProducts().stream()
						.map(orderApprovalEventProduct -> com.food.ordering.system.kafka.order.avro.model.Product
								.newBuilder().setId(orderApprovalEventProduct.getId())
